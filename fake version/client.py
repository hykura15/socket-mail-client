...
def insertToDatabase(connection, _from, _subject, _content, _fileNames, _fileContents, num, folder):
    if(type(_content) == list):
        _content = '\r\n'.join(_content)
    cursor = connection.cursor()
    insertQuery = f'''
    INSERT INTO {folder} (status, [from], subject, content, fileNames, fileContents, id)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    '''
    dataToInsert = {
        'status': "Not Seen",
        ...
        for rule in rules:
            ...
            if filterType == "from":
                ...
                if any(keyword in email[2] or keyword in email[3] for keyword in rule["keywords"]):
                    shouldMove = True
            if shouldMove:
                insertToDatabase(connection, email[1], email[2], email[3], email[4], email[5], email[6], rule["folder"])
                check = True
                shouldMove = False
        if check == False:
            insertToDatabase(connection, email[1], email[2], email[3], email[4], email[5], email[6], "Inbox")
    ...
    if maxId != int(mailNumber):
        try:
            for num in range(maxId + 1, int(mailNumber) + 1, 1):
                ...
                insertToDatabase(connection, _from, _subject, _content, _fileNames, _fileContents, num, "MailBox")
        except socket.error as e:
            print(f"Socket error: {e}")
